...
const CustomCheckbox = ({name, expanded}: Props) => {
  ...
  const {selectedCategories ,setSelectedCategories} = useManageSelectedCategories()
  ...
  return (
    <button className={`${checked?'text-white':'text-[#747477]'} w-fit cursor-pointer transition-all ${expanded?'h-[30px]':'h-[0]'} `}>
      {expanded && <Checkbox onChange={(e)=>handleCheck(e)} value={checked} theme="bootstrap-checkbox">{name}</Checkbox>}
    </button>
  )
}
