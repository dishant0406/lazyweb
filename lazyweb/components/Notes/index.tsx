...
const NoteComponent = (props: Props) => {
  ...
  return (
    <div className="w-full relative min-h-[100vh] bg-gray">
      ...
      <div
        className={`h-[100vh] p-[2%] w-[20vw] bg-gray border-l border-white/10 fixed top-0 transition-all duration-1000 ${
      ...
      >
        <div className="w-full flex mb-[1vh] justify-between items-center">
          ...
          <div className="flex gap-[0.5rem]">
            <button
              onClick={generateKey}
              className="w-10 h-10 flex justify-center items-center rounded-lg bg-gray/80 text-white"
            >
              <MdOutlineAddToPhotos className="text-[20px]" />
            </button>
            <button
        ...
        <div className="flex flex-col gap-2">
          {keys.map((key, index) => {
            return (
              <div
                key={key}
                className={`p-3 transition-all flex justify-start px-[5%] duration-200 rounded-md ${
                  selectedKey === key
                    ? "bg-gray text-white"
                    : "bg-gray text-white opacity-50"
                }`}
              >
                <p
                  onClick={() => selectKey(key)}
                  className="w-[90%] cursor-pointer text-start truncate"
                >
