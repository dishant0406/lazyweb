...
import { QrCodeModal, ResourceCard, ResourceListBar } from "components";
...
const ResourceList = (props: Props) => {
  ...
  return (
    <div>
      ...
      <div
        className={`relative w-[100%] transition-all duration-300 ${
      ...
      >
        {resources.length >= 1 && (
          <Reorder.Group
            ...
            axis={"x"}
            onReorder={() => {}}
            className="z-[1] ml-[3rem] flex gap-[1rem] flex-wrap mt-[2rem]"
          >
            {resources.map((e) => {
              return (
          ...
          <AnimatePresence>
            {resources.length < 1 && !loading && (
              <motion.div
                ...
                animate={{ scale: 1 }}
                exit={{ scale: 0 }}
                className="w-[100%] mt-[2rem] flex items-center justify-center"
              >
                <div className="px-[3rem] py-[1rem] shadow-2xl bg-altGray rounded-[20px]">
                  <p className="text-[18px] text-white">
                    {selectedTab === "saved" &&
                      `Not Bookmarked even a single resource! ${emojiGenerator()}`}
                    {selectedTab === "my" &&
                      `Add your personal resource and get started! ${emojiGenerator()}`}
                    {selectedTab === "all" && "No Resources Found! "}
                    {selectedTab === "publish" &&
                      `This place is empty right now! ${emojiGenerator()}`}
                  </p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
          {loading && selectedTab === "my" && (
